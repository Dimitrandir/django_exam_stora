...
- [ ] Пълен UI/UX редизайн на `sales` модула (потребителят иска това
      конкретно преработено, не само closed bugs). Първоначална визия,
      описана от потребителя: екранът разделен на 2 — от една страна
      Tabulator (кошницата/текущата продажба) отгоре с търсачка, от другата
      страна бутони за бързо добавяне, свързани с категории (POS-style).
      Взаимстване на идеи от съществуващи POS програми — конкретни
      референции предстои да се уточнят. Гъвкавост/възможност за
      модификация на layout-а е изрично искане. Не е започнато проектиране
      на конкретния UI все още.
  - [x] **Подкатегории** (`Category.parent`, self-FK) — направено като
        подготовка за бутоните по категория/подкатегория в новия касиерски
        екран. Виж CLAUDE.md за архитектурата (cycle prevention,
        `get_descendant_ids()`).
