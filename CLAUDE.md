...
- **"Load into Revision/Delivery/Write-off/Scrap"** — bulk действие в
  ...
  - **Delivery/Write-off/Scrap**: `_delivery_items_table.html` чете
    ...
    точно кой ред.
- **Фискален принтер (Daisy Perfect S01) — реална интеграция, `STORA/sales/fiscal.py`.**
  Пусната само за CASH продажби засега (CARD/MIXED нямат мапнат фискален
  Payment type все още). ECRCommApp (Daisy-ов JSON API сървър за апарата) се
  пуска като subprocess САМО около всяка продажба и се спира веднага след
  — не постоянна услуга — защото RACS (отделната система за контрол на
  достъп на обекта) периодично иска СЪЩИЯ COM порт; потвърдено на живо, че
  двете не могат да го държат едновременно. Пълната история на откриването
  на протокола (кой команди, кои полета, кой код на грешка какво значи) е
  в `kasov_aparat.md` + `kasov_aparat_ECRCommApp_Guide.pdf` (и двата в
  корена на проекта) — четени преди да се пипа `fiscal.py`.
  - **Продажбата никога не се блокира от фискален провал.** `sales_add`
    вика `fiscal.attempt_print(sale)` СИНХРОННО (не Celery — касиерът чака
    хартията в ръка), но чак СЛЕД `sale.save()`/`formset.save()` —
    продажбата вече е в базата. `attempt_print` никога не хвърля грешка
    навън; записва резултата в `SaleAttributes.fiscal_status`
    (`NONE`/`PRINTED`/`FAILED`) + `fiscal_error`. При `FAILED`,
    `sale_details.html` показва грешката и бутон "Retry Fiscal Print"
    (`sale_fiscal_retry` view, permission `sales.change_saleattributes`).
  - **`TaxGroup.fiscal_letter`** (1 символ) мапва всяка данъчна група към
    буквата ѝ на апарата (потвърдено: Б=20%, Г=9% на пилотния апарат) —
    празно поле означава "тази група никога не минава през фискалния
    принтер". `fiscal._validate_items()` проверява ВСИЧКИ редове на
    продажбата за мапната буква, ПРЕДИ изобщо да отвори бон на апарата —
    грешна/липсваща ставка на истински фискален бон е правен/счетоводен
    проблем, не просто UI бъг, затова никога не гадае буква по подразбиране.
  - **`FiscalCounter`** (единствен ред, `select_for_update()`) генерира
    поредния номер за УНП (`<сериен номер>-OP<оператор>-<7 цифри>`,
    напр. `DY497635-OP01-0000001`) — формат потвърден от
    `kasov_aparat_ECRCommApp_Guide.pdf`, не измислен.
  - Един фиксиран оператор за всички продажби (`FISCAL_OPERATOR_NUM`/
    `FISCAL_OPERATOR_PASSWORD` в `.env`) — нарочно опростено (малък обект,
    1-2 касиера), не мапване STORA `Employee` ↔ апаратен оператор.
  - `FISCAL_ENABLED=False` по подразбиране (виж `.env.example`) — dev
    машина без реален апарат просто никога не се опитва да фискализира,
    CASH продажбите остават `fiscal_status='NONE'` (не грешка).
  - При провал СЛЕД успешно отворен бон (напр. `FDSaleItem` гръмне),
  ...
  - **"Fiscal Reports" линкът е преместен** от Sales navbar менюто в
    футъра на самия POS екран (`sale_add.html`), до бутона "Refund" —
    касиерът да може да пусне дневния отчет директно накрая на смяната,
    без да напуска касовия екран.
  - **Баркод на фискалния бон** (`FDPrintBarcode`, Code128, данни =
    `sale.pk`) се отпечатва автоматично на всяка CASH продажба, точно
    преди `FDTotalSum` (командата изисква отворен бон — не може да се
    отпечата отделно след затваряне). Целта: сканиране на бележката по-
    късно директно намира продажбата в "Refund a Sale" търсачката —
    `refund_find` вече третира чисто число в `q` като точен sale ID, така
    че не трябваше никаква бекенд промяна там, само отпечатването на
    самия баркод. Съзнателно НЕ пипнахме продуктовия скенер на касовия
    екран (`sale_add.html`) — сканиране на бележка работи само в "Refund
    a Sale" екрана засега, по изричен избор на потребителя.
  - **Екранът "Refund Sale #N" (сторно избор на редове) вече е Tabulator**
    — старият readonly+overlay клавиатура UX е заменен с обичайния
    `numberEditor` патърн (Enter commit, клампва към `[0, remaining]` при
    commit вместо да разчита на HTML5 `min`/`max`, които не блокират
    надвишена стойност). Нов бутон **"Refund Entire Receipt"** в
    toolbar-а (до Columns/Export) попълва Refund Qty Now на всеки ред с
    неговия `remaining` наведнъж — все още изисква избор на причина и
    ръчно "Confirm Refund" накрая, не submit-ва сам. Скритите
    `refund_qty_<id>` input полета се пресъздават от Tabulator данните
    точно преди submit (същия патърн като Deliveries `rebuildHiddenInputs`).
  - **"Refund a Sale" екранът** (`refund_find.html`) вече показва Дата и
    Час в отделни колони (бяха едно комбинирано поле), плюс бутон "Back
    to Sales" (връща в `sale_add`). Таблицата остава plain HTML (не
    Tabulator — не е искано изрично за този екран).
  - **"Refund #N" детайлната страница** (`refund_details.html`, изгледът
    на вече завършено сторно) също мина на Tabulator, огледално на
    `sale_details.html`.
