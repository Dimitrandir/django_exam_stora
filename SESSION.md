...
**Статус:** `sales` одитът + подкатегориите вече комитнати (3 комита).
Некомитнато: Фаза 2 стъпки 1-2 (данни + категориен панел на касата, виж
по-долу).
...
- Комит на Фаза 2 стъпки 1-2 (некомитнато). После стъпка 3 (checkout
  модал — В брой/С карта + Сума/Платено/Ресто калкулатор), после стъпка 4
  (количката на Tabulator, изисква отделно потвърждение преди старт).
