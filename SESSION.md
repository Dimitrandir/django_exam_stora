...
**Статус:** `sales` одитът + подкатегориите + Фаза 2 стъпки 1-2 вече
комитнати (5 комита). Некомитнато: Фаза 2 стъпка 3 (checkout модал, виж
по-долу).
...
- Комит на Фаза 2 стъпка 3 (некомитнато). После стъпка 4 — количката на
  Tabulator (най-рисковата, заменя работещ тестван код) — изисква отделно
  изрично "давай" преди старт, per по-ранната договорка с потребителя.
