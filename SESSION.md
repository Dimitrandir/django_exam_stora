...
**Статус:** Комитнато (1 feature commit на main — "Cashier screen redesign
(step 4): POS shortcut curation", плюс този docs commit).
...
**Тествано:**
- Автоматични тестове: 234/234 (`python manage.py test`) — нови класове
  ...
  `CategoryTogglePosViewTests` (products).
- На живо в браузъра (през временни `_qa_temp_pos*` акаунти + `_QA`-
  префиксирани продукти/категории, всички изтрити след теста): filtering
  по show_on_pos, top-3-recent подреждане (реална продажба → продуктът
  качва напред), pinned bar render + repeat-tap пагинация (Page 1/2 →
  Page 2/2 → обратно на Page 1/2), edit mode add/remove пълен цикъл,
  toggle бутоните в Products грида (през Tabulator `cellClick`,
  потвърдено в базата) и Categories списъка (plain JS fetch, потвърдено
  в базата).
...
- Стъпка 4 от Фаза 2 (количката на Tabulator) остава необсъждана тази
  сесия — изрично отложена, чака отделно "давай".

**Следваща стъпка (предложение):**
- Потребителят тества активно самостоятелно (pin-ва реални категории/
  продукти) — изчакай feedback преди да продължиш. Ако няма повече
  корекции по POS shortcuts, следващото по ROADMAP е Стъпка 4 (Tabulator
  количка) или `reports` одита (последния неодитиран app от Фаза 1).
