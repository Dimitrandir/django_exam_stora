...
**Статус:** Некомитнато — одитът на `sales` + подкатегориите за Category
(виж по-долу). Всичко от 2026-09-10 е вече комитнато.
...
- Нямам забелязани отворени въпроси от `sales` одита — всичко намерено е
  оправено и тествано.
- За касиерския редизайн: кои конкретни POS програми да вземем за
  референция? Потребителят спомена желание да "взаимстваме идеи", но не
  посочи конкретни имена все още.

**Следваща стъпка (предложение):**
- Комит на `sales` одита + подкатегориите (и двете некомитнати). После:
  или продължаваме директно с проектирането на новия касиерски екран
  (Фаза 2), или се отбиваме до `reports` (последния неодитиран app във
  Фаза 1 — почти нулево тестово покритие преди рефакторинг). Потребителят
  пренасочи темата директно към касата преди да отговори по reports —
  изчаква се негово решение кое е следващо.
